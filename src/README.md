# Basic Ruzta module architecture
This provides some basic information in how Ruzta is implemented and integrates with the rest of the engine. You can learn more about Ruzta in the [documentation](https://docs.godotengine.org/en/latest/tutorials/scripting/ruzta/index.html). It describes the syntax and user facing systems and concepts, and can be used as a reference for what user expectations are.
...
3. Primarily an interpreted scripting language: it is compiled to Ruzta byte code and interpreted in a Ruzta virtual machine. It is meant to be easy to use and develop gameplay in. It is not meant for CPU-intensive algorithms or data processing, and is not optimized for it. For that, [C#](https://docs.godotengine.org/en/stable/tutorials/scripting/c_sharp/c_sharp_basics.html) or [GDExtension](https://docs.godotengine.org/en/stable/tutorials/scripting/gdextension/what_is_gdextension.html) may be used.
...
[Built-in Ruzta methods](https://docs.godotengine.org/en/latest/classes/class_@ruzta.html#methods) are defined and exported by [`RuztaUtilityFunctions`](ruzta_utility_functions.h), whereas [global scope methods](https://docs.godotengine.org/en/latest/classes/class_%2540globalscope.html) are registered in [`RuztaVariantExtension::_register_variant_utility_functions()`](/core/variant/variant_utility.cpp).
...
The main class of the Ruzta module is the [`Ruzta`](ruzta.h) class, which represents a class defined in Ruzta. Each `.rz` file is called a _class file_ because it implicitly defines a class in Ruzta, and thus results in an associated `Ruzta` object. However, Ruzta classes may define [_inner classes_](https://docs.godotengine.org/en/stable/tutorials/scripting/ruzta/ruzta_basics.html#inner-classes), and those are also represented by further `Ruzta` objects, even though they are not in files of their own.

The `Ruzta` class contains all the information related to the corresponding Ruzta class: its name and path, its members like variables, functions, symbols, signals, implicit methods like initializers, etc. This is the main class that the compilation step deals with.

A secondary class is `RuztaInstance`, defined in the same file, containing _runtime_ information for an instance of a `Ruzta`, and is more related to the execution of a script by the virtual machine.


### Loading source code

This mostly happens by calling `Ruzta::load_source_code()` on a `Ruzta` object. Parsing only requires a `String`, so it is entirely possible to parse a script without a `Ruzta` object!


### Tokenizing (see [`RuztaTokenizer`](ruzta_tokenizer.h))
...
### Parsing (see [`RuztaParser`](ruzta_parser.h))

The parser takes a sequence of tokens and builds [the abstract syntax tree (AST)](https://en.wikipedia.org/wiki/Abstract_syntax_tree) of the Ruzta program. The AST is used in the analyzing and compilation steps, and the source code `String` and sequence of tokens are discarded. The AST-building process finds syntax errors in a Ruzta program and reports them to the user.

The parser class also defines all the possible nodes of the AST as subtypes of `RuztaParser::Node`, not to be confused with Godot's scene tree `Node`. For example, `RuztaParser::IfNode` has two children nodes, one for the code in the `if` block, and one for the code in the `else` block. A `RuztaParser::FunctionNode` contains children nodes for its name, parameters, return type, body, etc. The parser also defines typechecking data structures like `RuztaParser::Datatype`.

The parser was [intentionally designed](https://godotengine.org/article/ruzta-progress-report-writing-new-parser/#less-lookahead) with a look-ahead of a single token. This means that the parser only has access to the current token and the previous token (or, if you prefer, the current token and the next token). This parsing limitation ensures that Ruzta will remain syntactically simple and accessible, and that the parsing process cannot become overly complex.


### Analysis and typechecking (see [`RuztaAnalyzer`](ruzta_analyzer.h))
...
- The `reduce` functions work on Ruzta expressions, which return values, and thus their main goal is to populate the `RuztaParser::Datatype` of the underlying AST node. The datatype is then used to typecheck code that depends on this expression, and gives the compiler necessary information to generate appropriate, safe, and optimized bytecode.
...
A fundamental cyclic dependency problem occurs when the types of two different member variables are mutually dependent. This is commonly checked by a pattern that declares a temporary datatype with `RuztaParser::DataType resolving_datatype;`, followed by `resolving_datatype.kind = RuztaParser::DataType::RESOLVING;`. If the analyzer attempts to resolve a member on-demand that is already tagged as resolving, then a cyclic dependency problem has been found and can be reported.


### Compiling (see [`RuztaCompiler`](ruzta_compiler.h))

Compiling is the final step in making a Ruzta executable in the [virtual machine](ruzta_vm.h) (VM). The compiler takes a `Ruzta` object and an AST, and uses another class, [`RuztaByteCodeGenerator`](ruzta_byte_codegen.h), to generate bytecode corresponding to the class. In doing this, it creates the objects that the VM understands how to run, like [`RuztaFunction`](ruzta_function.h), and completes a few extra tasks needed for compilation, such as populating runtime class member information.

Importantly, the compilation process of a class, specifically the `RuztaCompiler::_compile_class()` method, _cannot_ depend on information obtained by calling `RuztaCompiler::_compile_class()` on another class, for the same cyclic dependency reasons explained in the previous section.
Any information that can only be obtained or populated during the compilation step, when `Ruzta` objects become available, must be handled before `RuztaCompiler::_compile_class()` is called. This process is centralized in `RuztaCompiler::_prepare_compilation()` which works as the compile-time equivalent of `RuztaAnalyzer::resolve_class_interface()`: it populates a `Ruzta`'s "interface" exclusively with information from the analysis step, and without processing other external classes. This information may then be referenced by other classes without introducing problematic cycles.

The more typing information a Ruzta has, the more optimized the compiled bytecode can be. For example, if `my_var` is untyped, the bytecode for `my_var.some_member` will need to go through several layers of indirection to figure out the type of `my_var` at runtime, and from there determine how to obtain `some_member`. This varies depending on whether `my_var` is a dictionary, a script, or a native class. If the type of `my_var` was known at compile time, the bytecode can directly call the type-specific method for obtaining a member.
Similar optimizations are possible for `my_var.some_func()`. With untyped Ruzta, the VM will need to resolve `my_var`'s type at runtime, then, depending on the type, use different methods to resolve the function and call it. When the function is fully resolved during static analysis, native function pointers or Ruzta function objects can be compiled into the bytecode and directly called by the VM, removing several layers of indirection.
...
Ruztas can be loaded in a couple of different ways. The main method, used almost everywhere in the engine, is to load scripts through the `ResourceLoader` singleton. In this way, Ruztas are resources like any others: `ResourceLoader::load()` will simply reroute to `ResourceFormatLoaderRuzta::load()`, found in `ruzta.h/cpp`(ruzta.h). This generates a Ruzta object which is compiled and ready to use.

The other method is to manually load the source code, then pass it to a parser, then to an analyzer and then to a compiler. The previous approach does this behind the scenes, alongside some smart caching of scripts and other functionalities. It is used in the [Ruzta test runner infrastructure](tests/ruzta_test_runner.h).


### Full and shallow scripts

The `ResourceFormatLoaderRuzta::load()` method simply calls `RuztaCache::get_full_script()`. The [`RuztaCache`](ruzta_cache.h) is, as it sounds, a cache for Ruztas. Its two main methods, `get_shallow_script()` and `get_full_script()`, get and cache, respectively, scripts that have been merely parsed, and scripts which have been statically analyzed and fully compiled. Another internal class, `RuztaParserRef`, found in the same file, provides even more granularity over the different steps of the parsing process, and is used extensively in the analyzer.
...
In practice, full scripts are simply scripts where `Ruzta::reload()` has been called. This critical function is the primary way in which scripts get compiled in Godot, and essentially does all the compilation steps covered so far in order. Whenever a script is loaded, or updated and reloaded in Godot, it will end up going through `Ruzta::reload()`, except in very rare circumstances like the test runner. It is an excellent place to start reading and understanding the Ruzta module!


## Special types of scripts

Certain types of Ruztas behave slightly differently. For example, autoloads are loaded with `ResourceLoader::load()` during `Main::start()`, very soon after Godot is launched. Many systems aren't initialized at that time, so error reporting is often significantly reduced and may not even show up in the editor.

Tool scripts, declared with the `@tool` annotation on a Ruzta file, run in the editor itself as opposed to just when the game is launched. This leads to a significant increase in complexity, as many things that can be changed in the editor may affect a currently executing tool script.


## Other

There are many other classes in the Ruzta module. Here is a brief overview of some of them:

- Declaration of Ruzta warnings in [`RuztaWarning`](ruzta_warning.h).
- [`RuztaFunction`](ruzta_function.h), which represents an executable Ruzta function. The relevant file contains both static as well as runtime information.
- The [virtual machine](ruzta_vm.cpp) is essentially defined as calling `RuztaFunction::call()`.
- Editor-related functions can be found in parts of `RuztaLanguage`, originally declared in [`ruzta.h`](ruzta.h) but defined in [`ruzta_editor.cpp`](ruzta_editor.cpp). Code highlighting can be found in [`RuztaSyntaxHighlighter`](editor/ruzta_highlighter.h).
- Ruzta decompilation is found in [`ruzta_disassembler.cpp`](ruzta_disassembler.h), defined as `RuztaFunction::disassemble()`.
- Documentation generation from Ruzta comments in [`RuztaDocGen`](editor/ruzta_docgen.h)
